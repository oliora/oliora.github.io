...
In this post I will analize a piece of C code [automatically vectorized](https://en.wikipedia.org/wiki/Automatic_vectorization) by GCC and improve its performance by doing manual vectorization with using C++ and SIMD intrinsic functions.
...
Unless said otherwise all the assembly below is generated by compiling the source code on [Compiler Explorer](https://godbolt.org/) using **x86-64 gcc 13.2** with flags `-std=c++20 -O3 -ffast-math -Wall -DNDEBUG -march=x86-64-v3`. [`x86-64-v3`](https://en.wikipedia.org/wiki/X86-64#Microarchitecture_levels) means a "generic" x86-64 CPU with microarchitecture level 3 that has AVX2 but no AVX-512. Building for more specific architecture would surely generate more optimized code but for our case the generated code *essentially* would be the same. Note that benchmark code was compiled for specific architecture.

To understand the code parts you will need a bit of a knowledge on C, C++ and assembler. Here is a compact [refresher](https://docs.google.com/document/d/1hwABu-SL6u2cdhVkjOCUU6T0UyALujep1NfwYfQzWSo) for the latter.
...
Here you can find the generated [assembly](https://godbolt.org/z/zr3nbcKff) for the code above. If you want, you can compare it with [assembly of `naiveTableInt`](https://godbolt.org/z/nYqxG76aK) - the fastest naive soultion that uses table to avoid branching. If assembler scares you, don't worry, I'll walk you through the generated code in a moment.
...
.L9:
        ...
        vpsrldq xmm1, xmm0, 8               ;
        vpaddb  xmm0, xmm0, xmm1            ;
        vpsadbw xmm0, xmm0, xmm5            ; Horizontally sum each consequitive 8 absolute differences [1]
        vpextrb r8d, xmm0, 0                ; Take the lowest byte of the result and convert it from signed char to int
        movsx   r8d, r8b                    ;
...
1. [vpsadbw](https://www.intel.com/content/www/us/en/docs/intrinsics-guide/index.html#text=_mm256_sad_epu) horizontally sums each consequitive
   8 absolute differences (here it is `|counters[i] - nulls[i]|`) and put results into 4 16-bit integers.
   This is the way to sum 32 8-bit **unsigned** integers in one operation. After this one needs to sum up the resulting four 16-bit integers together to get a single number.



TODO:
- drawing
- explanation why using `short` step counter degrades performance
...
Note that the code for Intel Xeon was compiled with AVX-512 instructions but without allowing compiler to use 512-bit registers (no `-mprefer-vector-width=512` flag).

Benchmarks were run for two inputs:

1. `wp.txt` - War and Peace by Leo Tolstoy, 3.2 MiB
1. `long.txt` - `wp.txt` concatenated 100 times, 320 MiB

Benchmarks was run multiple time in a row for each algorithm and input combination: 10 runs of 10 passes per run for `wp.txt` and 0 runs of 1 pass per run `long.txt`. The best (max speed) result was choosen.

Benchmarks were run pinned to an isolated core.
...
1. Analyzing auto-vectorized code when 512-bit auto-vectorization is enabled (`-mprefer-vector-width=512` and AVX-512 [feature flags](https://gcc.gnu.org/onlinedocs/gcc/x86-Options.html))
...
3. x86-64 Assembly Language Summary - https://docs.google.com/document/d/1hwABu-SL6u2cdhVkjOCUU6T0UyALujep1NfwYfQzWSo
4. x86 and amd64 instruction reference - https://www.felixcloutier.com/x86/
5. Latency tables - https://www.uops.info/table.html and http://instlatx64.atw.hu/
6. Agner Fog's manuals on low level software optimization, contains a latency table and a lot of useful links - https://www.agner.org/optimize/
7. Top-Down performance analysis methodology by Denis Bakhvalov - an introduction into Top-down Microarchitecture Analysis Method (TMAM) - https://easyperf.net/blog/2019/02/09/Top-Down-performance-analysis-methodology
8. Intel 64 and IA-32 Architectures Optimization Reference Manual- http://www.intel.com/content/www/us/en/architecture-and-technology/64-ia-32-architectures-optimization-manual.html
